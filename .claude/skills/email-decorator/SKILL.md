...
## ¿Decorar el AlertaRepository?

`AlertaRepository` también implementa `IRepository[AlertaRiesgo]`, por lo que **técnicamente puede ser decorado**. Sin embargo, hay que pensarlo:

- Las alertas se crean automáticamente desde los `BusinessServices` cuando hay un PHQ-9 o GAD-7 severo.
- Cuando se crea una alerta, los `BusinessServices` **ya envían un email manualmente** notificando al psicólogo.
- Si decoramos `AlertaRepository`, se enviaría **un segundo email** redundante.

**Decisión recomendada**: NO decorar `AlertaRepository`. Solo decorar los 4 repositorios de las entidades CRUD (Estudiante, PHQ9, GAD7, Sesion), que son los que el criterio 7 exige cubrir.

```python
# Wiring completo en main.py
email = EmailService(modo_simulacion=True)
alerta_repo = AlertaRepository()                          # SIN decorar

# Los 4 repositorios CRUD sí van decorados
estudiante_repo = NotificacionDecorator(
    EstudianteRepository(), email, "bienestar@uni.edu", "Estudiante"
)
phq9_repo = NotificacionDecorator(
    PHQ9Repository(), email, "bienestar@uni.edu", "CuestionarioPHQ9"
)
# ... etc para GAD7 y Sesion
```
