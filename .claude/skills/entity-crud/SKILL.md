...
## Visibilidad de métodos en los modelos (importante)

Los modelos tienen métodos **privados** y **públicos** bien diferenciados. Esta separación se refleja también en el diagrama UML (atributos privados con `-`, públicos con `+`).

### Métodos PRIVADOS (con `_` al inicio)
- `_validar()`: dispara todas las validaciones del modelo. Se llama desde `__post_init__`.
- `_calcular_puntaje()`: solo en cuestionarios. Suma las respuestas.
- `_clasificar_severidad()`: solo en cuestionarios. Mapea puntaje a categoría clínica.

Nadie fuera del modelo debe llamar estos métodos directamente. Son detalles internos.

### Métodos PÚBLICOS (sin `_`)
- `to_dict()`: serializa la entidad a diccionario para guardar en JSON.
- `from_dict(data)`: classmethod que reconstruye la entidad desde un diccionario.

Estos son los únicos métodos que el repositorio y otras capas usan.
...
class CuestionarioPHQ9:
    ...
    def _validar(self) -> None:                  # ← privado
        if len(self.respuestas) != 9:
            raise ValidationError("Deben ser 9 respuestas.")
        for r in self.respuestas:
            if not (0 <= r <= 3):
                raise PuntajeInvalidoError(r, (0, 3))

    def _calcular_puntaje(self) -> int:          # ← privado
        return sum(self.respuestas)

    def _clasificar_severidad(self) -> str:      # ← privado
        if self.puntaje_total >= 20: return "Severa"
        if self.puntaje_total >= 15: return "Mod. Severa"
        if self.puntaje_total >= 10: return "Moderada"
        if self.puntaje_total >= 5: return "Leve"
        return "Mínima"

    def to_dict(self) -> dict:                   # ← público
        ...

    @classmethod
    def from_dict(cls, data: dict) -> "CuestionarioPHQ9":   # ← público
        ...
```
