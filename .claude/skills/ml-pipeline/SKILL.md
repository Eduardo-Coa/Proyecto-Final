...
**Ground truth**: derivado del puntaje total (suma de respuestas) según los rangos clínicos del PHQ-9. Esto significa que:
- El modelo **NO descubre nada nuevo** sobre clasificación clínica.
- Pero **demuestra el dominio técnico** del pipeline ML (preprocesamiento → split → entrenamiento → evaluación → persistencia).
- **El valor agregado** está en que el modelo puede predecir riesgo basado en respuestas parciales o detectar patrones de items específicos.

Esto es **totalmente válido y defendible**: muchos sistemas reales aprenden a replicar reglas conocidas para luego acelerar el procesamiento.
...
Botón "Predecir riesgo" en la vista de PHQ-9 que use el predictor:

```python
def _on_predecir(self):
    respuestas = [int(e.get()) for e in self._entries_respuestas]
    try:
        predictor = PHQ9Predictor()
        resultado = predictor.predecir(respuestas)
        self._mostrar_estado(
            f"Predicción: {resultado['clase_predicha']} "
            f"(confianza: {max(resultado['probabilidades'].values()):.0%})",
            exito=True,
        )
    except ModeloNoEntrenadoError as e:
        self._mostrar_estado(str(e), exito=False)
```

## Tests del modelo (forman parte del criterio 6)

```python
def test_predictor_clasifica_respuestas_todas_cero_como_minima():
    predictor = PHQ9Predictor()
    resultado = predictor.predecir([0]*9)
    assert resultado["clase_predicha"] == "Mínima"


def test_predictor_clasifica_respuestas_todas_tres_como_severa():
    predictor = PHQ9Predictor()
    resultado = predictor.predecir([3]*9)
    assert resultado["clase_predicha"] == "Severa"


def test_predictor_rechaza_respuestas_invalidas():
    predictor = PHQ9Predictor()
    with pytest.raises(ValueError):
        predictor.predecir([5, 0, 0, 0, 0, 0, 0, 0, 0])  # 5 fuera de rango


def test_entrenar_genera_modelo_persistido(tmp_path):
    # Entrenar y verificar que el archivo existe
    metricas = entrenar()
    assert RUTA_MODELO.exists()
    assert metricas["accuracy"] > 0.7  # Esperado alto porque la regla es determinística
```
...
# 3. Correr la app (el modelo se carga automáticamente)
python main.py
```
...
6. **No mostrar nunca raw probabilities sin contexto** en la UI — el usuario espera "confianza" en %, no "0.7341".
7. **Evaluar con métricas múltiples**: accuracy sola engaña en problemas desbalanceados. Usar F1 macro también.
